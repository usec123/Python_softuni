...
def product_quantity_ordered():
    result = []
    orders = Product.objects.exclude(orderproduct__quantity=None).values('name')\
        .annotate(quantity=Sum('orderproduct__quantity'))\
            .values('name', 'quantity')\
            .order_by('-quantity')
    for order in orders:
        result.append(f'Quantity ordered of {order["name"]}: {order["quantity"]}')
    return '\n'.join(result)
...
def give_discount():
    ...
    return '\n'.join(
        f'{product.name}: {product.price}lv.' for product in available_products
    )
